...
import { useLazyGetLoanReportQuery } from "../api/reportEndPoint";
...
import { LoanReportTableColumns } from "../utils/LoanReportTableColumns";
...
const LoanReport = () => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "loan_report",
    pageStyle: landscapePageStyle,
    removeAfterPrint: true,

    // onBeforeGetContent: <GlobalLoader />,
  });
  const loan_report_content = (
    <>
      <Table
        bordered
        columns={LoanReportTableColumns()}
          ...
          return (
            <Table.Summary.Row>
              <Table.Summary.Cell index={5} colSpan={4}>
                ...
                >
                  Total Loan Amount ( {data?.data?.summary.total_loan_amount} )
                </Typography.Text>
              </Table.Summary.Cell>
              ...
              <Table.Summary.Cell index={3}>
                <Typography.Text strong>
                  ...
                  >
                    Total Paid Amount ( {data?.data?.summary.total_paid_amount}{" "}
                    )
                  </div>
                </Typography.Text>
              </Table.Summary.Cell>
              <Table.Summary.Cell index={4}>
                <Typography.Text strong>
                  ...
                  >
                    Total Due Amount ( {data?.data?.summary.total_remaining_due}{" "}
                    )
                  </div>
                </Typography.Text>
              </Table.Summary.Cell>
                  ...
                  excelData={
                    data?.data?.data
                      ? data?.data?.data?.map((sData, index) => {
                          const data = {
                            SL: index + 1,
                            "Loan From": sData?.loan_from,
                            "Loan Amount": sData?.loan_amount,
                            "Installment Date": dayjs(
                              sData?.installment_date
                            ).format("YYYY-MM-DD"),
                            "Installment Amount": sData?.installment_amount,
                            "Remaining Due": sData?.remaining_due,
                          };
                          return data;
                        })
                      : []
