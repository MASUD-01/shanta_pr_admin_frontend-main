...
import { SourceTableColumns } from '../utils/SourceTableColoumns';
import { useSearchParams } from 'react-router-dom';
import { ISourceParams } from '../types/SourceTypes';
import { useEffect, useState } from 'react';
import { debounce } from 'lodash';
import { useGetSourceQuery } from '../api/sourceEndPoint';
...
const Sourcelist = () => {
  ...
  const sourceSub = configList?.sub_modules?.find(
    (i: any) => i?.sub_module_name === 'Source'
  );
  ...
  const [filter, setFilter] = useState<ISourceParams>({
    limit: 100,
    skip: skipValue - 100,
  });
  const { data, isLoading, isFetching } = useGetSourceQuery({ ...filter });
  useEffect(() => {
        ...
        <Card>
          ...
          >
            ...
            >
              <Typography.Title level={5} style={{ margin: 0 }}>
                {`Source List (${data?.total || 0})`}
              </Typography.Title>
              {sourceSub?.permissions?.write && (
          ...
          >
            <Table
              ...
              columns={SourceTableColumns(sourceSub)}
              scroll={{ x: true }}
              onChange={(pagination) => {
                setSearchParams({
