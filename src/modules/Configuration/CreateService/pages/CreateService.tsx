...
import { useGetCreateServiceQuery } from '../api/CreateServiceEndPoint';
import { CreateServiceTableColumns } from '../utils/CreateServiceTableColumns';
...
const CreateService = () => {
  ...
  const { data, isLoading } = useGetCreateServiceQuery({ ...filter });
  const showModal = () => {
    dispatch(
      setCommonModal({
        title: 'Create Service',
        content: <CreateServiceComponent />,
        show: true,
        width: 700,
      })
    );
  ...
  return (
    <>
      <div>
        <Card
          title={`Create Service List (${data?.total || 0})`}
          style={{
        ...
        >
          ...
          >
            <Table
              ...
              columns={CreateServiceTableColumns()}
              scroll={{ x: true }}
              onChange={(pagination) => {
                setSearchParams({
